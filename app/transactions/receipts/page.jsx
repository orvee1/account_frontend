...
import { createReceipt, deleteReceipt, fetchReceipts, updateReceipt } from '@/services/transactions';
...
export default function ReceiptsPage() {
    ...
    const loadReceipts = async () => {
        try {
            ...
            const mapped = data.map((receipt) => ({
                ...
                paymentMode: fromApiPaymentMode(receipt.payment_mode),
                reference: receipt.reference_number || '',
                description: receipt.description || '',
                status: receipt.status || 'completed',
                customer: receipt.customer?.name || receipt.customer_name || '',
            }));
            setReceipts(mapped);
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        ...
        const payload = {
            ...
            amount_received: formData.amount,
            payment_mode: toApiPaymentMode(formData.paymentMode),
    ...
    const handleDelete = (id) => {
        if (confirm('Are you sure you want to delete this receipt?')) {
            deleteReceipt(id).then(() => loadReceipts());
        }
    };
                ...
                <div className="flex flex-1">
                    <main className="flex-1 overflow-auto">
                        <div className="p-4 md:p-6">
                            ...
                            <div className="mb-6">
                                <div>
                                    <p className="text-gray-600 mt-1">Manage money received transactions</p>
                                </div>
                            </div>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                                <div className="bg-white rounded-lg shadow p-6">
                                    <p className="text-gray-600 text-sm font-medium mb-1">Total Receipts</p>
                                    <p className="text-2xl font-bold text-gray-900">{receipts.length}</p>
                                </div>
                                <div className="bg-white rounded-lg shadow p-6">
                                    <p className="text-gray-600 text-sm font-medium mb-1">Total Amount</p>
                                    <p className="text-2xl font-bold text-blue-600">
                                        ${receipts.reduce((sum, r) => sum + r.amount, 0).toLocaleString('en-US', {
                                        ...
                                        {filteredReceipts.map((receipt) => (
                                            ...
                                            >
                                                ...
                                                <td className="px-6 py-4 text-center">
                                                    <span
                                                        className={`px-3 py-1 rounded-full text-xs font-semibold ${receipt.status === 'completed'
