...
const EMPTY_BALANCE_SHEET = {
    ...
    | IMPORTANT:
    |
    | Backend ReportController returns:
    |
    | equity.currentProfitLoss
    |
    | Previously the frontend default object did not contain that property.
    | mergeAmounts() only copies keys that exist in the default object, so the
    | backend current profit/loss was silently discarded.
    |
    | currentProfitLoss is now part of the normalized Balance Sheet.
    |
    */
    equity: {
        commonStock: 0,
...
function normalizeReportData(
    ...
    const assets = {
        current: mergeAmounts(
            EMPTY_BALANCE_SHEET.assets.current,
            apiData.assets?.current
        ),

        fixed: mergeAmounts(
            EMPTY_BALANCE_SHEET.assets.fixed,
            apiData.assets?.fixed
        ),

        other: mergeAmounts(
            EMPTY_BALANCE_SHEET.assets.other,
            apiData.assets?.other
        ),
    };

    const liabilities = {
        current: mergeAmounts(
            EMPTY_BALANCE_SHEET.liabilities.current,
            apiData.liabilities?.current
        ),

        longTerm: mergeAmounts(
            EMPTY_BALANCE_SHEET.liabilities.longTerm,
            apiData.liabilities?.longTerm
        ),
    };

    /*
     * currentProfitLoss is now preserved here because it exists
     * in EMPTY_BALANCE_SHEET.equity.
     */
    const equity = mergeAmounts(
        EMPTY_BALANCE_SHEET.equity,
        apiData.equity
    );
...
|--------------------------------------------------------------------------
|
| This single structure is used by:
|
| 1. Browser UI
| 2. Printable report
| 3. Excel export
|
| Therefore adding Current Profit / Loss here automatically makes it
| available in all three output formats.
|
*/

function getBalanceSheetColumns(
    reportData
) {
    return [
        /*
                ...
                */

                {
                    ...
                    lines: [
                         ...
                         * NEW:
                         *
                         * Current financial-year profit/loss.
                         *
                         * Positive value increases equity.
                         * Negative value reduces equity.
                         */
                        {
                            label:
                                'Current Profit / Loss',
    ...
    const handleExport =
        async () => {
            ...
            try {
                ...
                alert(
                    'Balance Sheet export failed. Please try again.'
                );
            }
        };
...
function SummaryBox({
    ...
    return (
        <div
            className={`mb-6 rounded-lg border-2 p-4 print:mb-4 print:p-3 ${classes.box}`}
        >
            <div className="flex flex-wrap items-center justify-between gap-3">
                ...
                <span
                    className={`text-lg font-bold print:text-sm ${classes.amount}`}
                >
                    {formatCurrency(
                        amount
                    )}
