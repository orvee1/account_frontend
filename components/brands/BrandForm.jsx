...
export default function BrandForm({ onSave, onCancel, initial = null, isEdit = false, error = '' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setErrors({});
    if (!name.trim()) {
      setErrors({ name: ["The brand name is required."] });
      return;
    }
    setSubmitting(true);
    const payload = {
    ...
    toast({ title: isEdit ? "Brand Updated" : "Brand Created", description: name });
    onCancel();
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      ...
      <div>
        ...
        <Input value={name} onChange={(e) => setName(e.target.value)} placeholder="e.g., HealWell" />
        {err("name") && <p className="text-xs text-red-600 mt-1">{err("name")}</p>}
      </div>

      <div>
        <Label className="mb-1 block">Slug</Label>
        <Input value={slug} onChange={(e) => setSlug(e.target.value)} placeholder="healwell" />
        {err("slug") && <p className="text-xs text-red-600 mt-1">{err("slug")}</p>}
      </div>

      <div>
        <Label className="mb-1 block">Description</Label>
        <Textarea value={description} onChange={(e) => setDescription(e.target.value)} placeholder="Short brand description..." />
